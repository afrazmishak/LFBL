...
if ($result) {
    ...
    $existing_details_query = "SELECT * FROM `organization_userdetails` WHERE `organization_user_id`='$user_id'";
...
if (isset($_POST["submitDetails"])) {
    ...
    if (empty($existing_details)) {
        ...
        $sql = "INSERT INTO `organization_userdetails` (`firstname`, `lastname`, `dateofbirth`, `primaryphone`, `secondaryphone`, `email`, `address`, `zipcode`, `imagelocation` , `organization_user_id`) 
            VALUES ('$firstname', '$lastname', '$dateofbirth', '$primaryphone', '$secondaryphone', '$email', '$address', '$zipcode', '$imagelocation', '$user_id')";
    } else {
        // Update data in database for existing user
        $sql = "UPDATE `organization_userdetails` SET `firstname`='$firstname', `lastname`='$lastname', `dateofbirth`='$dateofbirth', `primaryphone`='$primaryphone', `secondaryphone`='$secondaryphone', `email` = '$email', `address`='$address', `zipcode`='$zipcode'";
        // Update image location if new image is uploaded
        if (!empty($imagelocation)) {
            $sql .= ", `imagelocation`='$imagelocation'";
        }
        $sql .= " WHERE `organization_user_id`='$user_id'";
    }
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        echo "<script>
                ...
                function closePopup() {
                    window.location.href = 'organization_dashboard.php'; // Redirect to home.php
                }
              </script>";
    } else {
...
<body>
    <div class="wrapper">
        ...
        <div class="form-box">
            <form class="login-container" id="login" action="./organizationuserdetails.php" method="POST"
